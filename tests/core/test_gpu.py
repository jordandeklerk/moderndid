...
requires_cupy = pytest.mark.skipif(not _HAS_GPU, reason="CuPy not installed or no CUDA GPU")
...
class TestBackendDispatch:
    ...
    def test_set_backend_cupy_without_gpu(self):
        if not HAS_CUPY or _HAS_GPU:
...
class TestGpuWlsNumpy:
    """Test gpu_wls on NumPy backend against known solutions."""
    ...
    def test_simple_ols(self):
        ...
        beta_hat, fitted = gpu_wls(y, X, weights)
    ...
    def test_gpu_vs_cpu_wls(self):
        ...
        set_backend("numpy")
        beta_cpu, fitted_cpu = gpu_wls(y, X, weights)

        import cupy as cp

        set_backend("cupy")
        beta_gpu, fitted_gpu = gpu_wls(cp.asarray(y), cp.asarray(X), cp.asarray(weights))
...
class TestGpuLogisticNumpy:
    """Test gpu_logistic_irls on NumPy backend against known solutions."""
    ...
    def test_separable_data(self):
        ...
        beta, mu = gpu_logistic_irls(y, X, weights)
    ...
    @requires_cupy
    def test_gpu_vs_cpu_logistic(self):
        ...
        set_backend("numpy")
        beta_cpu, mu_cpu = gpu_logistic_irls(y, X, weights)

        import cupy as cp

        set_backend("cupy")
        beta_gpu, mu_gpu = gpu_logistic_irls(cp.asarray(y), cp.asarray(X), cp.asarray(weights))
        beta_gpu = to_numpy(beta_gpu)
...
class TestMultiplierBootstrapGpu:
    ...
    @requires_cupy
    def test_gpu_bootstrap_shape(self):
        set_backend("cupy")
        rng = np.random.default_rng(42)
        inf_func = rng.standard_normal((100, 3))
        result = multiplier_bootstrap(inf_func, biters=50, random_state=42)
    ...
    @requires_cupy
    def test_gpu_vs_cpu_bootstrap(self):
    ...
    @requires_cupy
    def test_gpu_vs_cpu_bootstrap_1d(self):
...
class TestMbootGpu:
    ...
    @requires_cupy
    def test_gpu_vs_cpu_mboot(self):
        rng = np.random.default_rng(42)
        n = 200
        k = 3
    ...
    @requires_cupy
    def test_gpu_vs_cpu_mboot_clustered(self):
        rng = np.random.default_rng(42)
        n_units = 120
        n_clusters = 20
...
class TestAggregateByClusterGpu:
    ...
    @requires_cupy
    def test_gpu_vs_cpu_aggregate(self):
        rng = np.random.default_rng(42)
        n, k = 200, 5
        inf_func = rng.standard_normal((n, k))
    ...
    @requires_cupy
    def test_gpu_vs_cpu_aggregate_unequal_clusters(self):
        rng = np.random.default_rng(77)
...
class TestComputeColumnStdGpu:
    ...
    @requires_cupy
    def test_gpu_vs_cpu_column_std(self):
        rng = np.random.default_rng(42)
    ...
    @requires_cupy
    def test_gpu_column_std_with_nans(self):
        rng = np.random.default_rng(42)
