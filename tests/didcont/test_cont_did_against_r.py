...
def load_r_data():
    df = pd.read_csv("tests/didcont/cont_did_data.csv")
    df.loc[df["G"] == 0, "D"] = 0
    return df
...
def test_slope_dose_with_r_data():
    ...
    assert att_diff < 0.1, f"ATT difference {att_diff:.4f} exceeds tolerance"
    assert acrt_diff < 0.2, f"ACRT difference {acrt_diff:.4f} exceeds tolerance"
...
#     print("Python dynamic effects (slope):", result.event_study.att_by_event)
#     for e, att in zip(result.event_study.event_times, result.event_study.att_by_event):
#         r_val = r_dynamic.get(e)
#         if r_val is not None:
#             dynamic_diff = abs(att - r_val)
