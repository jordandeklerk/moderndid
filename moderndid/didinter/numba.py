...
def _compute_cluster_sums_py(influence_func, cluster_ids, unique_clusters):
...
def _multiplier_bootstrap_py(inf_func, weights_matrix):
...
def _aggregate_by_cluster_py(inf_func, cluster, unique_clusters):
...
def _gather_bootstrap_indices_py(sampled_cluster_ids, cluster_starts, cluster_counts):
    """Gather row indices for bootstrap sample."""
    total_rows = 0
...
def _compute_column_std_py(matrix):
...
if HAS_NUMBA:
    ...
    @nb.njit(cache=True)
    def _compute_cluster_sums_nb(influence_func, cluster_ids, unique_clusters):
        """Compute sum of influence function values within each cluster."""
        n_clusters = len(unique_clusters)
    ...
    def _multiplier_bootstrap_nb(inf_func, weights_matrix):
        """Multiplier bootstrap core computation with pre-generated weights."""
    ...
    @nb.njit(cache=True)
    def _aggregate_by_cluster_nb(inf_func, cluster, unique_clusters):
        """Aggregate influence functions by cluster."""
        n_clusters = len(unique_clusters)
        n, k = inf_func.shape
        cluster_mean_if = np.zeros((n_clusters, k))
    ...
    @nb.njit(cache=True)
    def _gather_bootstrap_indices_nb(sampled_cluster_ids, cluster_starts, cluster_counts):
        """Gather row indices for bootstrap sample."""
        total_rows = 0
        for cid in sampled_cluster_ids:
            total_rows += cluster_counts[cid]
    ...
    @nb.njit(cache=True)
    def _compute_column_std_nb(matrix):
        """Compute column-wise standard deviation ignoring NaNs."""
        n_rows, n_cols = matrix.shape
        result = np.empty(n_cols)
...
_compute_cluster_sums_impl = _compute_cluster_sums_nb if HAS_NUMBA else _compute_cluster_sums_py
_multiplier_bootstrap_impl = _multiplier_bootstrap_nb if HAS_NUMBA else _multiplier_bootstrap_py
_aggregate_by_cluster_impl = _aggregate_by_cluster_nb if HAS_NUMBA else _aggregate_by_cluster_py
_gather_bootstrap_indices_impl = _gather_bootstrap_indices_nb if HAS_NUMBA else _gather_bootstrap_indices_py
_compute_column_std_impl = _compute_column_std_nb if HAS_NUMBA else _compute_column_std_py
...
def multiplier_bootstrap(inf_func, nboot, random_state=None):
    ...
    weights_matrix = rng.binomial(1, p_kappa, size=(nboot, n)).astype(np.int8)
