"""Streaming cell computation for distributed DDD on Spark."""
...
from moderndid.dask._did_streaming import _partition_compute_did_rc_if, _precompute_did_rc_corrections
...
def streaming_cell_single_control(
    ...
    All computation stays on the driver via collected partition dicts.
    The influence function matrix ``inf_func_mat`` is updated in-place.
    ...
    part_data_list : list of dict or None, default None
        Pre-built partition data from the wide-pivot path. When provided,
    ...
    if part_data_list is not None:
        ...
        part_data_list, n_cell = prepare_cell_partitions(
            spark,
            sdf,
            g,
            t,
    ...
    ps_betas, or_betas = streaming_nuisance_coefficients(spark, part_data_list, est_method, k)
    ...
    for pd in part_data_list:
        ids_part, if_part = _partition_compute_ddd_if(
            pd,
            ps_betas,
            or_betas,
            global_agg,
...
def streaming_cell_multi_control(
    ...
    subsetting is done via numpy masks (no per-control Spark shuffles).
    ...
    full_filter = (F.col(group_col) == 0) | (F.col(group_col) > max_period) | (F.col(group_col) == g)
    filtered = sdf.filter(full_filter)
    ...
    cached_cell_df = merged_sdf.repartition(n_partitions).cache()
    try:
        ...
        for pdf in cached_cell_df.toLocalIterator(prefetchPartitions=True):
            ...
            if part is not None:
                ...
                all_part_data_list.append(part)
    finally:
        cached_cell_df.unpersist()

    if not all_part_data_list:
        return None
    ...
    for ctrl in available_controls:
        ctrl_part_data_list = [_filter_partition_for_ctrl(pd, g, ctrl) for pd in all_part_data_list]
        ctrl_part_data_list = [p for p in ctrl_part_data_list if p is not None]

        if not ctrl_part_data_list:
            continue
        first = ctrl_part_data_list[0]
        if first is None:
            continue
        k = first["X"].shape[1]

        ps_betas, or_betas = streaming_nuisance_coefficients(spark, ctrl_part_data_list, est_method, k)
        ...
        n_subset = sum(p["n"] for p in ctrl_part_data_list if p is not None)
        n3 = global_agg[3]["n_sub"]
        n2 = global_agg[2]["n_sub"]
        n1 = global_agg[1]["n_sub"]
...
def streaming_ddd_rc_cell_single_control(
    ...
    """
    part_data_list, n_cell = _prepare_ddd_rc_cell_partitions(
        spark,
        sdf,
        g,
        t,
    ...
    def _fit_comparison(comp_sg):
        if est_method == "reg":
            ...
            ps_b = distributed_logistic_irls_from_partitions(
                spark,
                part_data_list,
        ...
        if est_method == "ipw":
            ...
            for d_val, post_val, key in [
                ...
                or_b[key] = distributed_wls_from_partitions(
                    spark,
                    part_data_list,
        ...
        stats_list = [
            _partition_ddd_rc_global_stats(pd, comp_sg, ps_b, or_b, est_method, trim_level) for pd in part_data_list
        ]
        agg = _driver_reduce_stats(stats_list)
    ...
    scale = n_obs / n_cell
    for pd in part_data_list:
        ids_part, if_part = _partition_compute_ddd_rc_if(
            pd,
            ps_betas,
            or_betas_all,
            global_aggs,
...
def streaming_ddd_rc_cell_multi_control(
    ...
    if len(available_controls) == 1:
        ...
        group_filter = (F.col(group_col) == ctrl) | (F.col(group_col) == g)
    ...
    all_part_data_list, n_all = _prepare_ddd_rc_cell_partitions(
        spark,
        filtered,
        g,
        t,
    ...
    if all_part_data_list is None or n_all == 0:
        return None
    ...
    for ctrl in available_controls:
        ctrl_part_data_list = [_filter_rc_partition_for_ctrl(pd, g, ctrl) for pd in all_part_data_list]
        ctrl_part_data_list = [p for p in ctrl_part_data_list if p is not None]
        ...
        result = _ddd_rc_single_from_parts_collect(spark, ctrl_part_data_list, est_method, trim_level)
        if result is None:
            continue
...
def prepare_cell_partitions(
    ...
    """Filter and merge post/pre periods, returning a list of partition dicts.

    Performs the data preparation for one :math:`(g, t)` cell. Filters the
    Spark DataFrame to relevant groups (treated cohort plus controls),
    merges each unit's post-period and pre-period outcomes via a join,
    repartitions, caches, and converts each partition to a numpy dict
    via ``_build_partition_arrays``.
    ...
    part_data_list : list of dict or None
        List of numpy dicts from ``_build_partition_arrays``,
...
def streaming_nuisance_coefficients(spark, part_data_list, est_method, k):
    ...
    All estimation is performed on the driver via partition dicts; only
    :math:`k`-dimensional coefficient vectors are produced.
    ...
    part_data_list : list of dict
        Partition dicts from ``_build_partition_arrays``.
    est_method : {"dr", "reg", "ipw"}
    ...
    def _fit_one(comp_sg):
        ...
        if est_method == "ipw":
            ...
            or_b = distributed_wls_from_partitions(
                spark,
                part_data_list,
...
def streaming_global_stats(spark, part_data_list, ps_betas, or_betas, est_method, trim_level=0.995):
    r"""Compute global aggregate statistics and precomputed correction vectors.

    For each comparison subgroup :math:`c \in \{3, 2, 1\}`, performs a
    driver-side reduce to aggregate per-partition sufficient statistics into
...
# ---------------------------------------------------------------------------
# Pure numpy partition-level helpers (copied exactly from Dask)
# ---------------------------------------------------------------------------
...
def _prepare_ddd_rc_cell_partitions(
    ...
    """Concatenate post/pre periods for DDD RC, returning a list of partition dicts."""
    if not pre_filtered:
        max_period = max(t, pret)
        if control_group == "nevertreated":
            group_filter = (F.col(group_col) == 0) | (F.col(group_col) == g)
    ...
    concat_sdf = concat_sdf.withColumn("_post", (F.col(time_col) == t).cast("int"))
...
def _ddd_rc_single_from_parts(spark, part_data_list, n_cell, n_obs, inf_func_mat, counter, est_method, trim_level):
    """Run DDD RC estimation from pre-built part_data_list and write to inf_func_mat."""
    if not part_data_list:
        return None
    first = part_data_list[0]
    if first is None:
...
def _ddd_rc_single_from_parts_collect(spark, part_data_list, est_method, trim_level):
    ...
    n_total = sum(p["n"] for p in part_data_list if p is not None)
    n3, n2, n1 = global_aggs[3]["n_sub"], global_aggs[2]["n_sub"], global_aggs[1]["n_sub"]
...
def _driver_reduce_stats(stats_list):
    """Reduce a list of per-partition stats dicts on the driver using sum_global_stats."""
    filtered = [s for s in stats_list if s is not None]
    if not filtered:
        return None
    return reduce(sum_global_stats, filtered)
