...
def ddd_mp(
    ...
    For each (g,t) cell with comparison group :math:`g_c`, the doubly robust
    estimand (Equation 4.8 from [1]_) is

    .. math::
        \widehat{ATT}_{dr,g_c}(g,t) &= \mathbb{E}_n\left[(\widehat{w}_{trt}^{S=g,Q=1}
            - \widehat{w}_{g,0}^{S=g,Q=1})(Y_t - Y_{g-1}
            - \widehat{m}_{Y_t-Y_{g-1}}^{S=g,Q=0}(X))\right] \\
        &+ \mathbb{E}_n\left[(\widehat{w}_{trt}^{S=g,Q=1}
            - \widehat{w}_{g_c,1}^{S=g,Q=1})(Y_t - Y_{g-1}
            - \widehat{m}_{Y_t-Y_{g-1}}^{S=g_c,Q=1}(X))\right] \\
        &- \mathbb{E}_n\left[(\widehat{w}_{trt}^{S=g,Q=1}
            - \widehat{w}_{g_c,0}^{S=g,Q=1})(Y_t - Y_{g-1}
            - \widehat{m}_{Y_t-Y_{g-1}}^{S=g_c,Q=0}(X))\right].
