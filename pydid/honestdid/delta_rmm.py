...
def _compute_conditional_cs_rmm_fixed_s(
    ...
    if post_period_moments_only and num_post_periods > 1:
        ...
        rows_for_arp = _find_rows_with_post_period_values(A_rmm, post_period_indices)
    else:
        rows_for_arp = None
...
if HAS_NUMBA:
    ...
    def _find_rows_with_post_period_values(A_rmm, post_period_indices):
        """Find rows with non-zero values in post-period columns using Numba."""
        rows = []
        for i in range(A_rmm.shape[0]):
            has_nonzero = False
    ...
    def _find_rows_with_post_period_values(A_rmm, post_period_indices):
        ...
        has_post_period_values = np.any(A_rmm[:, post_period_indices] != 0, axis=1)
